...
class QueryBuilder {
  ...
  search() {
    if (
      ...
      this.mongoQuery.$or = this.config.searchFields.map((field) => ({
        [field]: searchRegex,
      }));
    }
  ...
  filter() {
    ...
    Object.keys(queryObj).forEach((key) => {
      ...
      if (
        ...
        !this.config.allowedFields.includes(key.split("[")[0])
      ) {
      ...
      if (operatorMatch) {
        ...
        // Validate operator
        if (
          this.config.allowedOperators &&
          !this.config.allowedOperators.includes(operator)
        ) {
    ...
    switch (operator) {
      ...
      case "contains":
        this.mongoQuery[field] = {
          $regex: new RegExp(this.escapeRegex(value), "i"),
        };
        break;
  ...
  sort() {
    if (this.queryString.sort) {
      ...
      sortFields.forEach((field) => {
        ...
        if (
          this.config.allowedFields &&
          !this.config.allowedFields.includes(fieldName.trim())
        ) {
          return; // Skip non-whitelisted fields
        }
    ...
    if (selectParam) {
      ...
      if (this.config.allowedFields) {
        const validFields = fields.filter((f) =>
          this.config.allowedFields.includes(f)
        );
  ...
  paginate() {
    ...
    const limit =
      parseInt(this.queryString.limit) || this.config.defaultLimit || 50;
